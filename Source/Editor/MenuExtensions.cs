namespace Spotlight.Editor
{
    ...
    internal static class MenuExtensions
    {
        ...
        /// Allows to input a new value by selecting the item.
        /// </summary>
        public static UIMenuNumericScrollerItem<byte> WithTextEditing(this UIMenuNumericScrollerItem<byte> item, int maxLength = 32)
            => WithTextEditing(item, maxLength, byte.TryParse, "8-bit integer number");
        ...
        private static T WithTextEditingBase<T>(T item, int maxLength, Func<string> strGetter, Action<string> resultCallback) where T : UIMenuItem
        {
            ...
            item.Activated += (m, s) =>
            {
                Plugin.EditorMenuPool.Draw();

                const string TitleId = "SPOTLIGHT_ONSCREEN_KEYBOARD_TITLE";
                Localization.SetText(TitleId, $"{item.Text.Trim()}:");
                NativeFunction.Natives.DISPLAY_ONSCREEN_KEYBOARD(6, TitleId, "", strGetter(), "", "", "", maxLength);
                int state;
                ...
                Localization.ClearTextOverride(TitleId);
